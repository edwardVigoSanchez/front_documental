...
@Component({
  selector: 'app-list-memorandum',
  templateUrl: './list-memorandum.component.html',
...
export class ListMemorandumComponent {
  ...
  private getTableData(): void {
    ...
    this.memorandumService.listUsers().subscribe((resp:any) => {
  ...
  deleteUser() {
    this.memorandumService.deleteUser(this.memorandum_selected.id).subscribe((resp: any) => {
      console.log(resp);
      let INDEX = this.usersList.findIndex((item: any) => item.id == this.memorandum_selected.id);
      if (INDEX != -1) {
