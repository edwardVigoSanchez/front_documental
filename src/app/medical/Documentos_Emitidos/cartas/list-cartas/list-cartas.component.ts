...
export class ListCartasComponent {
  ...
  private getTableData(): void {
    ...
    this.cartasService.listUsers().subscribe((resp:any) => {
  ...
  deleteUser() {
    this.cartasService.deleteUser(this.cartas_selected.id).subscribe((resp: any) => {
      console.log(resp);
      let INDEX = this.usersList.findIndex((item: any) => item.id == this.cartas_selected.id);
      if (INDEX != -1) {
