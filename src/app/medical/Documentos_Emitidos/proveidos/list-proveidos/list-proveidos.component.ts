...
export class ListProveidosComponent {
  ...
  private getTableData(): void {
    ...
    this.proveidosService.listUsers().subscribe((resp:any) => {
  ...
  deleteUser() {
    this.proveidosService.deleteUser(this.proveidos_selected.id).subscribe((resp: any) => {
      console.log(resp);
      let INDEX = this.usersList.findIndex((item: any) => item.id == this.proveidos_selected.id);
      if (INDEX != -1) {
