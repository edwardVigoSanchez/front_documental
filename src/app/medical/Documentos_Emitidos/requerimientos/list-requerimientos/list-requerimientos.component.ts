...
export class ListRequerimientosComponent {
  ...
  private getTableData(): void {
    ...
    this.requerimientosService.listUsers().subscribe((resp:any) => {
  ...
  deleteUser() {
    this.requerimientosService.deleteUser(this.requerimientos_selected.id).subscribe((resp: any) => {
      console.log(resp);
      let INDEX = this.usersList.findIndex((item: any) => item.id == this.requerimientos_selected.id);
      if (INDEX != -1) {
        ...
        this.requerimientos_selected = null;
      }
    })
  }
