...
@Component({
  ...
  styleUrl: './list-informes.component.scss'
})
export class ListInformesComponent {
  ...
  ngOnInit() {
    this.getTableData();
    this.user = this.informesService.authService.user;
  }
  private getTableData(): void {
    this.usersList = [];
    this.serialNumberArray = [];

    this.informesService.listUsers().subscribe((resp:any) => {
  ...
  deleteUser() {
    this.informesService.deleteUser(this.informes_selected.id).subscribe((resp: any) => {
      console.log(resp);
      let INDEX = this.usersList.findIndex((item: any) => item.id == this.informes_selected.id);
      if (INDEX != -1) {
