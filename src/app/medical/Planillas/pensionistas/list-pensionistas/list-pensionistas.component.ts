...
@Component({
  ...
  templateUrl: './list-pensionistas.component.html',
  styleUrl: './list-pensionistas.component.scss'
})
export class ListPensionistasComponent {
  ...
  private getTableData(): void {
    ...
    this.pensionistasService.listUsers().subscribe((resp:any) => {
  ...
  selectUser(rol:any){
    this.pensionistas_selected = rol;
  }

  deleteUser() {
    this.pensionistasService.deleteUser(this.pensionistas_selected.id).subscribe((resp: any) => {
      console.log(resp);
      let INDEX = this.usersList.findIndex((item: any) => item.id == this.pensionistas_selected.id);
      if (INDEX != -1) {
        ...
        this.pensionistas_selected = null;
      }
    })
  }
