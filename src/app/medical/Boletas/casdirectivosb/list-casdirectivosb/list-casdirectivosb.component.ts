...
@Component({
  ...
  templateUrl: './list-casdirectivosb.component.html',
...
export class ListCasdirectivosbComponent {
  ...
  selectUser(rol:any){
    this.casdirectivosb_selected = rol;
  }

  deleteUser() {
    this.casdirectivosbService.deleteUser(this.casdirectivosb_selected.id).subscribe((resp: any) => {
      console.log(resp);
      let INDEX = this.usersList.findIndex((item: any) => item.id == this.casdirectivosb_selected.id);
      if (INDEX != -1) {
        ...
        this.casdirectivosb_selected = null;
      }
    })
  }
